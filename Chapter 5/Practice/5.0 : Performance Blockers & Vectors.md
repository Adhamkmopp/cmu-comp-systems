...
Similar to loop unrolling, this method attemps to exploit the unused copies of functional units. The code below performs an operation on the odd and even indices of the vector in parallel.
...
n operations are still performed, but independandtly in two's with each loop round.
